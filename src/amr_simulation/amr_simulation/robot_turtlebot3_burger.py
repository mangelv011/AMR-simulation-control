...
class TurtleBot3Burger(Robot):
    ...
    def move(self, v: float, w: float) -> None:
        ...
        b = self.TRACK / 2
        phi_l = (v-b*w) / self.WHEEL_RADIUS
        phi_r = (v+b*w) / self.WHEEL_RADIUS

      
        max_speed = max(abs(phi_l), abs(phi_r))
        if max_speed > self.WHEEL_SPEED_MAX:
            scale = self.WHEEL_SPEED_MAX / max_speed
            phi_l *= scale
            phi_r *= scale
    ...
    def _sense_encoders(self) -> tuple[float, float]:
        ...
        z_v =   (self.WHEEL_RADIUS*(w_left + w_right)) / 2
        z_w = (self.WHEEL_RADIUS*(w_right - w_left)) / self.TRACK
